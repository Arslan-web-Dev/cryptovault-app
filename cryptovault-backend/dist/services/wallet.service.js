...
class WalletService {
    async createWallet(request) {
        ...
        const existingWallet = await database_1.prisma.wallet.findFirst({
            where: {
        ...
        if (existingWallet) {
            throw (0, error_middleware_1.createError)('You already have a wallet of this type', 409);
        }
        // Create blockchain wallet
        let walletData;
        switch (request.coinType) {
            case 'ETH':
                walletData = await blockchain_service_1.default.createEthereumWallet();
                break;
            case 'BTC':
                walletData = await blockchain_service_1.default.createBitcoinWallet();
                break;
            case 'SOL':
                walletData = await blockchain_service_1.default.createSolanaWallet();
                break;
            case 'USDT':
                walletData = await blockchain_service_1.default.createEthereumWallet(); // USDT uses Ethereum addresses
                break;
            default:
                throw (0, error_middleware_1.createError)('Unsupported coin type', 400);
        }
        // Encrypt private key
        const encryptedPrivateKey = blockchain_service_1.default.encryptPrivateKey(walletData.privateKey);
        // Check if this should be default wallet
        const userWallets = await database_1.prisma.wallet.findMany({
        ...
        const walletsWithBalances = await Promise.all(wallets.map(async (wallet) => {
            try {
                ...
                switch (wallet.coin_type) {
                    case 'ETH':
                        balanceData = await blockchain_service_1.default.getEthereumBalance(wallet.wallet_address);
                        break;
                    case 'BTC':
                        balanceData = await blockchain_service_1.default.getBitcoinBalance(wallet.wallet_address);
                        break;
                    case 'SOL':
                        balanceData = await blockchain_service_1.default.getSolanaBalance(wallet.wallet_address);
                        break;
                    case 'USDT':
                        balanceData = await blockchain_service_1.default.getUSDTBalance(wallet.wallet_address);
                        break;
                    default:
    ...
    async deleteWallet(walletId, userId) {
        ...
        if (wallet.balance > 0) {
            throw (0, error_middleware_1.createError)('Cannot delete wallet with non-zero balance', 400);
        }
    ...
    async getWalletTransactions(walletId, userId, page = 1, limit = 10) {
        const wallet = await database_1.prisma.wallet.findFirst({
            where: {
                id: walletId,
                user_id: userId
    ...
    async validateAddress(address, coinType) {
        return blockchain_service_1.default.validateAddress(address, coinType);
    }
